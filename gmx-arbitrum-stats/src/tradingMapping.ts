...
import { OpenInterestByToken, TradingStat } from "../generated/schema";
import { updatePriceFeeds } from "./pricesMapping";

let ZERO = BigInt.fromI32(0);

function _loadOrCreateEntity(
  id: string,
  period: string,
  timestamp: BigInt
): TradingStat {
...
function _loadOrCreateOpenInterestByToken(
  id: string,
  period: string,
  timestamp: BigInt,
  indexToken: Address
...
function _updateOpenInterestByToken(
  timestamp: BigInt,
  increase: boolean,
  isLong: boolean,
  delta: BigInt,
  ...
  const entity = _loadOrCreateOpenInterestByToken(
    id,
    "total",
    timestamp,
    indexToken
  ...
  const dayEntity = _loadOrCreateOpenInterestByToken(
    dayId,
    "daily",
    dayTimestamp,
    indexToken
...
function _updateOpenInterest(
  ...
  if (isLong) {
    totalEntity.longOpenInterest = increase
      ? totalEntity.longOpenInterest + delta
      : totalEntity.longOpenInterest - delta;
  } else {
    totalEntity.shortOpenInterest = increase
      ? totalEntity.shortOpenInterest + delta
      : totalEntity.shortOpenInterest - delta;
  }
  totalEntity.save();
...
function _storePnl(
  ...
  entity.liquidatedCollateralCumulative =
    totalEntity.liquidatedCollateralCumulative;
  entity.save();
}
