...
export function handleUniswapGmxEthSwap(event: UniswapSwap): void {
  ...
  let totalId = CAD
  _storeUniswapPrice(totalId, CAD, gmxPrice, "last", event.block.timestamp, event.block.number)

  let id = CAD + ":" + event.block.timestamp.toString()
  _storeUniswapPrice(id, CAD, gmxPrice, "any", event.block.timestamp, event.block.number)
}
