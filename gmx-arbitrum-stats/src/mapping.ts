...
export function handleAddLiquidity(event: AddLiquidity): void {
  _storeClpStat(event.block.timestamp, event.params.glpSupply, event.params.aumInUsdg)
}
...
export function handleDistributeEsgmxToGmx(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(CAD, amount)
...
function _getOrCreateClpStat(id: string, period: string): ClpStat {
...
export function handleDistributeEthToClp(event: Distribute): void {
  ...
  let totalEntity = _getOrCreateClpStat("total", "total")
  totalEntity.distributedEth += amount
  totalEntity.distributedEthCumulative += amount
  totalEntity.distributedUsd += amountUsd
  totalEntity.distributedUsdCumulative += amountUsd
  ...
  let id = _getDayId(event.block.timestamp)
  let entity = _getOrCreateClpStat(id, "daily")
...
export function handleDistributeEsgmxToClp(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(CAD, amount)
...
function _storeClpStat(timestamp: BigInt, glpSupply: BigInt, aumInUsdg: BigInt): void {
  let deprecatedId = _getHourId(timestamp)
  let deprecatedEntity = HourlyClpStat.load(deprecatedId)

  if (deprecatedEntity == null) {
    deprecatedEntity = new HourlyClpStat(deprecatedId)
